"""Research implementation V3 (Windows-GPU variant) — claim-relevant run for dissertation defence.

This is the native-Windows + NVIDIA-GPU copy of research_imp_V3_gpu.py. TensorFlow dropped
native-Windows GPU support after TF 2.10, so this variant targets the TF-2.10 / Python-3.10
stack (see research_imp/requirements_windows_gpu.txt and WINDOWS_GPU_RUN_INSTRUCTIONS.md).

It differs from research_imp_V3_gpu.py ONLY in:
- Models are saved/loaded as .h5 instead of .keras. The Keras-3 .keras archive format does not
  exist in TF 2.10; .h5 (HDF5) is supported by both TF 2.10 (Windows) and TF 2.15 (Linux).
