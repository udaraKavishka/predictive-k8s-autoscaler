"""Research implementation V3 (GPU variant) — claim-relevant run for dissertation defence.

This is a GPU-enabled copy of research_imp_V3.py. It differs ONLY in:
- configure_environment() enables the GPU (memory growth) instead of forcing CPU.
- Default config skips the multi-seed and ablation re-runs (run_multi_seed=False,
  run_ablation=False); their existing artifacts are kept as-is.
- All model/continual-learning logic is identical to research_imp_V3.py.

Stages run by default (the ones needed to defend the claims):
  train + baselines  -> hybrid model validation vs 4 baselines (Objective 1)
  forgetting + naive_ft -> proposed (EWC+ER) vs naive fine-tuning BWT contrast (Objective 2)
  cross_app -> generalisation limitation
  sla + dashboard -> SLA metrics and Grafana/evaluation figures

Inherited from V3:
- EWC lambda: 500 → 100 (allows chunk-4 adaptation; fixes proposed vs Periodic Retrain)
- Replay ratio: 0.4 → 0.2 (reduces old-data dominance)
- naive_ft stage: trains sequential model without EWC/ER for the BWT comparison
- Baselines: loaded from cache by default (no --force-baselines needed)

Run with:  python research_imp_V3_gpu.py --refresh-all
"""
...
# Duplicates stdout to a log file so every print and traceback is captured automatically.
class TeeLogger:
...
class ExperimentConfig:
    ...
    epochs: int = 30
    batch_size: int = 128
    # Separate epoch count for baseline/naive_ft models (comparison only, not the main model).
    ...
    # Validation controls (GPU variant).
    # NOTE: multi-seed and ablation are now ON by default. The earlier artifacts were
    # produced with the leaked instance_count feature, so they MUST be regenerated.
...
# Sets TensorFlow/NumPy randomness and enables the GPU for accelerated execution.
def configure_environment(seed: int) -> None:
    # Reduce verbose TensorFlow C++ logs (but do NOT hide the GPU on this variant).
    os.environ.setdefault("TF_CPP_MIN_LOG_LEVEL", "3")

    # GPU variant: detect available GPUs and enable incremental memory growth so the
    # process does not grab all VRAM at once. Falls back silently to CPU if none found.
    try:
        gpus = tf.config.list_physical_devices("GPU")
        if gpus:
            for gpu in gpus:
                tf.config.experimental.set_memory_growth(gpu, True)
            print(f"    GPU(s) detected   : {[g.name for g in gpus]} (memory growth enabled)")
        else:
    ...
    # Keep float32 precision (not mixed_float16) so results match the CPU run exactly
    # and stay numerically defensible for the dissertation claims.
...
def create_sequences(
    ...
    for i in range(total):
        ...
        # Take static features at the true "now" (last observed step), not one
        # bin into the future, to avoid a 5-minute lookahead leak.
...
# Minimal History-compatible object so resumed chunks have a usable .history dict.
...
def train_proposed_model(
    ...
    # Resume from the latest per-chunk checkpoint when available.
    start_chunk = 0
    if output_dir is not None and cfg.resume and not cfg.force_retrain and not cfg.refresh_all:
        for ck in range(len(chunks) - 1, -1, -1):
            ckpt_model = output_dir / f"proposed_chunk_{ck}.keras"
            ckpt_meta = output_dir / f"proposed_chunk_{ck}_meta.json"
            if ckpt_model.exists() and ckpt_meta.exists():
                model = tf.keras.models.load_model(ckpt_model)
                # Guard against stale checkpoints after a feature-set change (e.g. removing
                # instance_count): if the saved static input dim no longer matches, drop the
                # checkpoint and retrain from scratch instead of crashing on a shape mismatch.
                try:
                    ckpt_static_dim = int(model.inputs[1].shape[-1])
                except Exception:
                    ckpt_static_dim = -1
                if ckpt_static_dim != n_static:
                    print(
                        f"    [checkpoint] chunk {ck + 1} checkpoint is stale "
                        f"(static dim {ckpt_static_dim} != {n_static}) -> retraining from scratch"
                    )
                    model = create_hybrid_model(cfg.history_length, n_temporal, n_static)
                    ewc = EWC(model, fisher_multiplier=cfg.ewc_lambda)
                    break
                print(f"    [checkpoint] found chunk {ck + 1} checkpoint -> resuming")
                ewc = EWC(model, fisher_multiplier=cfg.ewc_lambda)
                with open(ckpt_meta, "r", encoding="utf-8") as _f:
                    _meta = json.load(_f)
                chunk_results = _meta["chunk_results"]
                bwt_matrix = {
                    int(k): {int(kk): float(vv) for kk, vv in v.items()}
                    for k, v in _meta["bwt_matrix"].items()
                }
                train_histories = [
                    _HistoryWrapper(h) for h in _meta["train_histories"]
                ]
                for prev_ck in range(ck + 1):
                    pc = chunks[prev_ck]
                    sp = int(0.8 * len(pc["y"]))
                    val_sets.append((pc["X_seq"][sp:], pc["X_static"][sp:], pc["y"][sp:]))
                    replay.update_memory(pc["X_seq"][:sp], pc["X_static"][:sp], pc["y"][:sp])
                last_c = chunks[ck]
                sp = int(0.8 * len(last_c["y"]))
                prev_x_seq = last_c["X_seq"][:sp].copy()
                prev_x_static = last_c["X_static"][:sp].copy()
                prev_y = last_c["y"][:sp].copy()
                start_chunk = ck + 1
                print(f"    [checkpoint] continuing from chunk {start_chunk + 1}/{cfg.n_chunks}")
                break
    ...
    for chunk_idx in range(start_chunk, len(chunks)):
        ...
        # Save per-chunk checkpoint so a crash only loses the current chunk's work.
        if output_dir is not None:
            _ckpt_model = output_dir / f"proposed_chunk_{chunk_idx}.keras"
            _ckpt_meta = output_dir / f"proposed_chunk_{chunk_idx}_meta.json"
            model.save(_ckpt_model)
            _meta_out = {
                ...
                "train_histories": [
                    {
                        "loss": h.history.get("loss", []) if hasattr(h, "history") else [],
                        "val_loss": h.history.get("val_loss", []) if hasattr(h, "history") else [],
                        "mae": h.history.get("mae", []) if hasattr(h, "history") else [],
                    }
                    for h in train_histories
                ],
            }
            with open(_ckpt_meta, "w", encoding="utf-8") as _f:
                json.dump(_meta_out, _f)
            print(f"    [checkpoint] chunk {chunk_idx + 1} saved -> {_ckpt_model.name}")
...
def run_baselines_with_cache(
    ...
    if (
        ...
        print("    [baseline][static_lstm] clearing GPU memory before training")
        ...
        try:
            ...
            static_lstm_model = create_lstm_only_model(cfg.history_length, n_temporal)
            static_lstm_model.fit(
                x_tr_seq, y_tr_c1, epochs=cfg.baseline_epochs, batch_size=cfg.batch_size, verbose=0
            )
            y_slstm_scaled = static_lstm_model.predict(x_final_seq, verbose=0).flatten()
            ...
            print(f"    [baseline][static_lstm] FAILED ({type(exc).__name__}: {exc}) — skipping")
            traceback.print_exc()
            preds["Static LSTM"] = np.full(len(y_final_real), np.nan)
            baseline_status["Static LSTM"] = "failed"
    ...
    if (
        ...
        print("    [baseline][static_hybrid] clearing GPU memory before training")
        tf.keras.backend.clear_session()
        gc.collect()
        try:
            ...
            print(f"    [baseline][static_hybrid] FAILED ({type(exc).__name__}: {exc}) — skipping")
            traceback.print_exc()
            preds["Static Hybrid"] = np.full(len(y_final_real), np.nan)
            baseline_status["Static Hybrid"] = "failed"
    ...
    if (
        ...
        print("    [baseline][periodic] clearing GPU memory before training")
        tf.keras.backend.clear_session()
        gc.collect()
        try:
            ...
            print(f"    [baseline][periodic] FAILED ({type(exc).__name__}: {exc}) — skipping")
            traceback.print_exc()
            preds["Periodic Retrain"] = np.full(len(y_final_real), np.nan)
            baseline_status["Periodic Retrain"] = "failed"
...
# Evaluates one trained model on every task (chunk) validation set and returns per-task MAE.
# This supports the continual-learning "average over all tasks" metric (ACC). It is the fair
# way to compare a forgetting-resistant model against naive/periodic retraining: the latter
# look good only on the final chunk because they forget the earlier ones.
def evaluate_model_all_tasks(
    ...
    maes: list[float] = []
    for x_seq_v, x_stat_v, y_v in val_sets:
        inp = [x_seq_v, x_stat_v] if hybrid else x_seq_v
        y_pred_s = model.predict(inp, verbose=0).flatten()
        y_pred = target_scaler.inverse_transform(y_pred_s.reshape(-1, 1)).flatten()
        y_real = target_scaler.inverse_transform(y_v.reshape(-1, 1)).flatten()
        maes.append(float(mean_absolute_error(y_real, y_pred)))
    return maes


# Computes the average-task MAE for the proposed model and the retrain-style baselines and
# writes avg_task_metrics.csv. The proposed (EWC+ER) hybrid is expected to win here, because
# the periodic/static baselines forget the earlier chunks while it retains them.
def compute_average_task_metrics(
    output_dir: Path,
    ...
    def add(name: str, model: Model | None, hybrid: bool) -> None:
        if model is None:
            return
        maes = evaluate_model_all_tasks(model, val_sets, target_scaler, hybrid)
        rows.append(
            {
                "Method": name,
                "AvgTaskMAE": float(np.mean(maes)),
                "FinalTaskMAE": float(maes[-1]),
                "PerTaskMAE": ";".join(f"{m:.2f}" for m in maes),
            }
        )

    add("Proposed (EWC+ER)", proposed_model, hybrid=True)
    add("Periodic Retrain", baseline_models.get("periodic_model"), hybrid=True)
    add("Static Hybrid", baseline_models.get("static_hybrid_model"), hybrid=True)
    add("Static LSTM", baseline_models.get("static_lstm_model"), hybrid=False)

    df = pd.DataFrame(rows)
    df.to_csv(output_dir / "avg_task_metrics.csv", index=False)
    return df
...
def run_naive_ft_comparison(
    ...
    # Resume from the latest per-chunk naive_ft checkpoint when available.
    start_chunk = 0
    if output_dir is not None and cfg.resume and not cfg.force_validation and not cfg.refresh_all:
        for ck in range(len(chunks) - 1, -1, -1):
            _ckpt = output_dir / f"naive_ft_chunk_{ck}.keras"
            _ckpt_meta = output_dir / f"naive_ft_chunk_{ck}_meta.json"
            if _ckpt.exists() and _ckpt_meta.exists():
                print(f"    [naive_ft][checkpoint] resuming from chunk {ck + 1}")
                model = tf.keras.models.load_model(_ckpt)
                with open(_ckpt_meta, "r", encoding="utf-8") as _f:
                    _meta = json.load(_f)
                diagonal_maes = _meta["diagonal_maes"]
                for prev_ck in range(ck + 1):
                    pc = chunks[prev_ck]
                    sp = int(0.8 * len(pc["y"]))
                    val_sets.append((pc["X_seq"][sp:], pc["X_static"][sp:], pc["y"][sp:]))
                start_chunk = ck + 1
                print(f"    [naive_ft][checkpoint] continuing from chunk {start_chunk + 1}/{cfg.n_chunks}")
                break
    ...
    for chunk_idx in range(start_chunk, len(chunks)):
        ...
        # Save per-chunk checkpoint so a crash only loses the current chunk's work.
        if output_dir is not None:
            _ckpt = output_dir / f"naive_ft_chunk_{chunk_idx}.keras"
            model.save(_ckpt)
            with open(output_dir / f"naive_ft_chunk_{chunk_idx}_meta.json", "w", encoding="utf-8") as _f:
                json.dump({"diagonal_maes": diagonal_maes}, _f)
            print(f"    [naive_ft][checkpoint] chunk {chunk_idx + 1} saved")
...
def config_signature(cfg: ExperimentConfig) -> str:
    ...
    sig_payload = {
        ...
        "sla_tol": cfg.sla_tol,
        "seed": cfg.seed,
        # Bump this whenever the feature set changes so all caches auto-invalidate.
        # v2: removed leaked "instance_count" static feature; static now taken at true "now".
        "feature_set_version": "v2_no_instance_count",
    }
...
def run_experiment(cfg: ExperimentConfig) -> dict[str, Any]:
    ...
    static_features = [
        ...
        "rdma_request_mean",
        "role_hn_fraction",
        # NOTE: "instance_count" was removed because it leaks the target.
        # cpu_demand = sum of active cpu_request ~= instance_count * mean_request,
        # so instance_count is a near-linear proxy of cpu_demand and lets the
        # MLP-only branch shortcut the forecast (MAE ~145), which unfairly beats
        # the hybrid. Removing it makes the architecture comparison honest.
        "max_instance_per_node",
    ]
    target = "cpu_demand"
    ...
    if "baselines" in selected_stages:
        ...
        # Continual-learning average-task metric: evaluate every final model on ALL chunks
        # seen so far, not just the last one. This is the fair comparison and is where the
        # proposed hybrid+EWC+ER model is expected to beat periodic/static retraining,
        # because those baselines forget the earlier chunks.
        try:
            if model is not None and val_sets:
                avg_df = compute_average_task_metrics(
                    output_dir, model, baseline_models, val_sets, target_scaler
                )
                print("    average-task MAE (lower is better, mean over all chunks seen):")
                print(avg_df.round(4).to_string(index=False))
                mark_artifact(
                    manifest, "avg_task_metrics", output_dir / "avg_task_metrics.csv"
                )
        except Exception as exc:
    ...
    if "naive_ft" in selected_stages:
        ...
        if (
            ...
            nft_result = run_naive_ft_comparison(
                cfg, chunks, n_temporal, n_static, target_scaler, output_dir=output_dir
            )
...
def parse_args() -> ExperimentConfig:
    ...
    return ExperimentConfig(
        ...
        # GPU variant: multi-seed, ablation and sensitivity are force-disabled here
        # (claim-relevant run only). Their prior artifacts are retained, not regenerated.
...
def main() -> None:
    ...
    # Start log capture before printing anything so all output is saved.
    log_path = Path(cfg.output_dir) / f"run_log_{time.strftime('%Y%m%d_%H%M%S')}.txt"
