"""Research implementation V3 (Linux-GPU variant) — claim-relevant run for dissertation defence.

This is the Linux + NVIDIA-GPU copy of the V3 pipeline, derived from research_imp_V3_win_gpu.py
with the cache-compatibility guard + naive_ft OOM fixes. It is intended to be run in a fresh
folder (no pre-existing .h5 artifacts) under a modern TF/Keras-3 stack (e.g. TF 2.21 / Keras
3.14 in conda env `research_env`).

IMPORTANT (Keras 2 -> 3): .h5 model files produced by the old TF-2.10 Windows runs CANNOT be
loaded by Keras 3 (e.g. "Unrecognized keyword argument 'time_major'"). The cache-compatibility
guard catches any such load failure and retrains, so when this folder starts empty everything
is trained cleanly from scratch in the current environment.

Notes:
- Models are saved/loaded as .h5. Keras 3 still writes/reads .h5 (legacy HDF5); models saved by
  THIS environment reload fine within it.
- configure_environment() enables the GPU (memory growth) instead of forcing CPU.
- Multi-seed and ablation are ON by default (run_multi_seed=True, run_ablation=True):
  the older artifacts came from the leaked instance_count run and must be regenerated.
- The "instance_count" static feature was removed (it leaked the target), and the
  average-task (continual-learning ACC) metric is written to avg_task_metrics.csv.
...
Run with:  python research_imp\research_imp_V3_win_gpu.py --refresh-all
"""
...
def run_experiment(cfg: ExperimentConfig) -> dict[str, Any]:
    ...
    if "train" in selected_stages:
        ...
        proposed_model_path = output_dir / "hybrid_model_ewc_er.h5"
...
def main() -> None:
    ...
    # Start log capture before printing anything so the whole run is saved to a file.
